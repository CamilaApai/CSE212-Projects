...
public static class SetsAndMapsTester
{
    ...
    private static void DisplayPairs(string[] words)
    {
        ...
        // Create a HashSet to store words and their symmetrical pairs
        var set = new HashSet<string>();

        // Iterate through the array of words
        foreach (string word in words)
        {
            // Create a string representing the symmetrical pair of the current word
            var pair = $"{word[1]}{word[0]}";

            // Check if the symmetrical pair of the current word exists in the HashSet
            if (set.Contains(pair))
            {
                // If it does, print the pair
                Console.WriteLine($"{word} & {pair}");
            }
            else
            {
                // If it doesn't, add the current word to the HashSet
                set.Add(word);
            }
        }
    }
